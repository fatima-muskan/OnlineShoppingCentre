...
const UpdateProduct = () => {
  ...
  const handleDeleteProduct = async()=>{
    try {
        let answer=window.prompt('Are you surely want to delete this product?')
        if(!answer) return
        const {data}=await axios.put(`/api/v1/product/delete-product/${id}`);
    ...
    <Layout title={'Create Product'}>
        ...
        <div className='row'>
            ...
            <div className='m-1 w-75'>
              <Select bordered={false} placeholder='Select Category' size='large' showSearch className='form-select mb-3' onChange={(value) => {setCategory(value)}} value={category}>
                {categories?.map(c =>(
